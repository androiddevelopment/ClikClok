...
public class TileUpdaterTest extends TestCase {
	...
	public void testUpdateColours()
	{
		...
		tileUpdated.setDirection(TileDirection.EAST);
		
		tileUpdater.updateColours(tileUpdated, tileStatus, TileColour.GREEN, TileColour.RED);	
		
		assertEquals(4, tileStatus.getTilesWithColours().get(TileColour.GREEN).size());
		assertEquals(TileColour.GREEN, tileStatus.getTiles()[1][0].getColour());
		assertEquals(TileColour.GREEN, tileStatus.getTiles()[2][0].getColour());
		assertEquals(TileColour.GREEN, tileStatus.getTiles()[2][1].getColour());
