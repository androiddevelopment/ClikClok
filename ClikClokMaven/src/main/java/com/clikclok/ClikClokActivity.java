...
import com.clikclok.util.CustomDialog;
...
public class ClikClokActivity extends RoboActivity implements UpdateUIListener{ 
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        levelLabelView.setTypeface(UIUtilities.setFont(getString(R.string.game_font_type)));
        levelNumberView.setTypeface(UIUtilities.setFont(getString(R.string.game_font_type)));
        userScoreView.setTypeface(UIUtilities.setFont(getString(R.string.game_font_type)));
        aiScoreView.setTypeface(UIUtilities.setFont(getString(R.string.game_font_type)));
        countDownTimerView.setTypeface(UIUtilities.setFont(getString(R.string.game_font_type))); 
	...
	protected Dialog onCreateDialog(int id) {
		...
		CustomDialog.Builder builder = new CustomDialog.Builder((Context)this, R.layout.dialog_layout);
		switch(id)
		{
			case Constants.LEVEL_COMPLETE_DIALOG:	 			
				builder.setMessage(getString(R.string.levelComplete, currentLevel.getLevelNum()))
				       .createLeftButton(getString(R.string.proceedButton), new View.OnClickListener() {
							@Override
			...
			case Constants.USER_WINNER_DIALOG:
				builder.setMessage(getString(R.string.userIsWinner))
				       .createLeftButton(getString(R.string.prizeButton), new View.OnClickListener() {
							@Override
							public void onClick(View v) {
								Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(getString(R.string.prizeURL)));
			...
			case Constants.AI_WINNER_DIALOG:
				builder.setMessage(getString(R.string.aiIsWinner))
				       .createLeftButton(getString(R.string.noButton), new View.OnClickListener() {
							@Override
							public void onClick(View v) {
								finish();						
							}				    	   
				       })
				       .createRightButton(getString(R.string.yesButton), new View.OnClickListener() {
				    	   @Override
							public void onClick(View v) {
				    		   initialize();
			...
			case Constants.INTRODUCTION_DIALOG:
				builder.setMessage(getString(R.string.introduction))
				       .createLeftButton(getString(R.string.startGameButton), new View.OnClickListener() {
				    	   @Override
							public void onClick(View v) {
				    		   dismissDialog(Constants.INTRODUCTION_DIALOG);
							}				    	   
				       })
				       .createRightButton(getString(R.string.rulesButton), new View.OnClickListener() {
							@Override
							public void onClick(View v) {
			...
			case Constants.RULES_DIALOG:
				builder.setMessage(getString(R.string.rules))
				       .createLeftButton(getString(R.string.okButton), new View.OnClickListener() {
						@Override
						public void onClick(View v) {
	...
	@Override
	public void updateGrid(final int userScore, final int aiScore) {
	...
	public void showNextLevelDialog(final Level currentLevel) {	
		...
		handler.post(new Runnable() {
			...
			public void run() {
				...
				levelNumberView.setText("" + Level.getNextLevel(currentLevel).getLevelNum());				
			}
		});
	...
	/**
	 * Width is size of tile times ten, minus one eight
	 * Height is size of tile times thirteen. Minus an eight does not have any impact here
	...
	private void initializeGridWidthAndHeight()
	{
		...
		int layoutHeight = Constants.TILE_WIDTH * Constants.GRID_HEIGHT * 15/16;
		int verticalSpacing = 0 - Constants.TILE_WIDTH / 8;
		Log.d(this.getClass().toString(), "Layout parameters are " + layoutWidth + ", " + layoutHeight + "," + verticalSpacing);
		gridView.setLayoutParams(new FrameLayout.LayoutParams(layoutWidth, layoutHeight));
		countDownTimerView.setLayoutParams(new FrameLayout.LayoutParams(layoutWidth, layoutHeight));
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		...
		{
			case Constants.LEVEL_COMPLETE_DIALOG:
				customDialog.setMessage((getString(R.string.levelComplete, currentLevel.getLevelNum())));
			break;
		}
		
	}
	
	// TODO all calls to this method should be originating from our uiOperationQueue
	@Override
	public void updateCountdownTimer(final String secondsLeft) {
		Log.d(this.getClass().toString(), "Entering updateCountdownTimer with " + secondsLeft + " seconds left");
		handler.post(new Runnable() {			
			@Override
			public void run() {
	...
	private void initialize() {
        ...
        setScores(0, 0); // This is required if the user wishes to restart the game
	}
	...
	private void stopGame() {
		...
		// TODO Could possibly add a dialog here
		finish();
	}
